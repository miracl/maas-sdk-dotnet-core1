...
namespace MiraclAuthenticationTests
{
    ...
    public class MiraclClientTests
    {
        ...
        [Test]
        public void Test_GetIdentityAsync_ParseUserInfoContainingJsonArray()
        {
            var mockHttp = new MockHttpMessageHandler();
            mockHttp.When(TokenEndpoint).Respond("application/json", "{\"access_token\":\"" + ValidAccessToken + "\",\"expires_in\":900,\"id_token\":\"" + ValidIdToken + "\",\"refresh_token\":\"MockRefresh\",\"scope\":\"openid\",\"token_type\":\"Bearer\"}");
            mockHttp.When(UserEndpoint).Respond("application/json", "{\"email\":[\"petya.koleva@miracl.com\", \"petya@example.com\"],\"sub\":\"petya.koleva@miracl.com\"}");

            MiraclClient client = InitClient(ValidClientId, "MockSecret", mockHttp);
            var response = client.ValidateAuthorizationCodeAsync("MockCode", "petya.koleva@miracl.com").Result;
            Assert.That(response, Is.Not.Null);
            ...
            Assert.That(identity.Claims.Count(c => c.Type.Equals("email")), Is.EqualTo(2));
            Assert.That(identity.Claims.Where(c => c.Type.Equals("email") && c.Value.Equals("petya.koleva@miracl.com")).Count(), Is.EqualTo(1));
            Assert.That(identity.Claims.Where(c => c.Type.Equals("email") && c.Value.Equals("petya@example.com")).Count(), Is.EqualTo(1));
        }
        ...
        [TestCase(ClaimValueTypes.Boolean, "true")]
        [TestCase(JsonClaimValueTypes.JsonArray, "[]")]
        [TestCase(ClaimValueTypes.DateTime, "\"\\/Date(1335205592410)\\/\"")]
        [TestCase(ClaimValueTypes.Double, "3.14")]
        [TestCase(ClaimValueTypes.Integer, "123")]
        [TestCase(ClaimValueTypes.Integer64, "12345678987654321")]
        [TestCase(JsonClaimValueTypes.Json, "{}")]
        [TestCase(ClaimValueTypes.String, "\"some string\"")]
        [TestCase(ClaimValueTypes.String, "undefined")]
        public void Test_GetClaimValueType(string expectedType, string value)
        {
            var client = new MiraclClient(new MiraclOptions());
            var jToken = JObject.Parse($"{{'key' : {value} }}").GetValue("key");
            Assert.AreEqual(expectedType, client.GetClaimValueType(jToken));
        }

        [Test]
        public void Test_GetClaimValueType_ShouldReturnJsonNullWhenPassedNullValue()
        {
            var client = new MiraclClient(new MiraclOptions());
            Assert.AreEqual(JsonClaimValueTypes.JsonNull, client.GetClaimValueType(null));
        }
