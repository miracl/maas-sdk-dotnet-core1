...
namespace MiraclAuthenticationTests
{
    ...
    public class MiraclHandlerTests
    {
        ...
        [Test]
        public void Test_HandleUnauthorizedAsync_NoOptions()
        {
            var settings = new TestSettings(null);
            Assert.That(() => settings.CreateTestServer(),
                Throws.TypeOf<ArgumentNullException>().And.Property("ParamName").EqualTo("options"));

        }

        [Test]
        public void Test_HandleUnauthorizedAsync_ChallengeIsIssuedCorrectly()
        {
            var settings = new TestSettings(
                new MiraclOptions()
                {
                    ...
                    ClientId = "Test Id",
                    ResponseType = "code",
                    BackchannelHttpHandler = GetMockHttpHandler(),
                    AutomaticChallenge = true,
                    ProtocolValidator = new OpenIdConnectProtocolValidator { RequireNonce = true }
                });
        ...
        [Test]
        public void Test_HandleUnauthorizedAsync_SetsNonceAndStateCookies()
        {
            var settings = new TestSettings(new MiraclOptions()
            {
                ClientId = "Test Id",
                Authority = TestServerBuilder.DefaultAuthority,
                BackchannelHttpHandler = GetMockHttpHandler()
        ...
        [Test]
        public void Test_HandleUnauthorizedAsync_Challenge_WithEmptyConfig_Fails()
        {
            var settings = new TestSettings(
                 new MiraclOptions()
                 {
                     ClientId = "Test Id With Empty Config",
