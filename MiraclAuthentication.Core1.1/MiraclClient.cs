...
namespace Miracl
{
    ...
    public class MiraclClient
    {
        ...
        private void AddUserInfoClaimsAsync(string claimsIssuer)
        {
            ...
            if (Options.GetClaimsFromUserInfoEndpoint)
            {
                foreach (var claim in identity.Claims)
                {
                    ...
                    JToken value;
                    var isClaimIncluded = this.UserJson.TryGetValue(claim.Type, out value) || this.UserJson.TryGetValue(shortClaimTypeName, out value);
                    ...
                    {
                        if (!this.UserJson.Remove(claim.Type))
                        {
                            this.UserJson.Remove(shortClaimTypeName);
                        }
                    }
                }
                ...
                foreach (var pair in UserJson)
                {
                    ...
                    if (array != null)
                    {
                        ...
                        {
                            identity.AddClaim(new Claim(pair.Key, item?.ToString() ?? string.Empty, GetClaimValueType(item), claimsIssuer));
                        }
                    }
                    else
                    {
                        identity.AddClaim(new Claim(pair.Key, pair.Value?.ToString() ?? string.Empty, GetClaimValueType(pair.Value), claimsIssuer));
                    }
                }
        ...
        // by default the token is signed with audience ClientId
